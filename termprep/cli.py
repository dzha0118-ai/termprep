...
@click.option("--ai", "use_ai", is_flag=True, help="\u542f\u7528 AI \u589e\u5f3a\u6a21\u5f0f\uff08\u9700\u914d\u7f6e API Key\uff09")
@click.option("--ai-engine", default=None, help="AI \u5f15\u64ce\u540d\u79f0: ai, google, youdao")
@click.option("--translate-mode", default="auto", help="\u7ffb\u8bd1\u6a21\u5f0f: auto | mixed | zh2en | en2zh")
def pipeline(file, text, project_name, top, search_limit, db_name, output, export_fmt, notes, use_ai, ai_engine, translate_mode):
    ...
    # Show AI-enhanced info
    if result.ai_used:
        console.print("\n[green]\u2713[/green] AI \u589e\u5f3a\u6a21\u5f0f\u5df2\u542f\u7528")
        if result.glossary_json:
            json_path = f"{project_name}-glossary.json"
            try:
                with open(json_path, "w", encoding="utf-8") as f:
                    f.write(result.glossary_json)
                console.print(f"[green]\u2713[/green] \u6807\u51c6\u5316 Glossary JSON \u5df2\u5bfc\u51fa: {json_path}")
            except Exception:
                pass
    else:
        console.print("\n[yellow]![/yellow] \u4f7f\u7528\u514d\u8d39\u6a21\u5f0f\uff08Google Translate + Youdao\uff09\n\u63d0\u793a: \u914d\u7f6e TERMPREP_AI_API_KEY \u53ef\u542f\u7528 AI \u589e\u5f3a")


# ============================================================
# ai-pipeline (shortcut)
# ============================================================
@main.command(name="ai-pipeline")
@click.argument("file", required=False)
@click.option("--text", "-t", default="", help="\u76f4\u63a5\u8f93\u5165\u6587\u672c")
@click.option("--name", "project_name", default="Untitled", help="\u9879\u76ee\u540d\u79f0")
@click.option("--top", default=20, help="\u63d0\u53d6\u672f\u8bed\u6570\u91cf", type=int)
@click.option("--db", "db_name", default=None, help="\u8bcd\u5e93\u540d\u79f0")
@click.option("--output", "-o", default="", help="\u62a5\u544a\u8f93\u51fa\u8def\u5f84")
@click.option("--export", "export_fmt", default="", help="\u5bfc\u51fa\u683c\u5f0f")
@click.option("--engine", default=None, help="AI \u5f15\u64ce\u540d\u79f0")
@click.option("--translate-mode", default="auto", help="\u7ffb\u8bd1\u6a21\u5f0f: auto | mixed | zh2en | en2zh")
def ai_pipeline(file, text, project_name, top, db_name, output, export_fmt, engine, translate_mode):
    """AI \u589e\u5f3a\u6d41\u6c34\u7ebf\u5feb\u6377\u547d\u4ee4\uff08\u7b49\u4ef7\u4e8e pipeline --ai\uff09\u3002"""
    if not file and not text:
        console.print("[red]\u9519\u8bef:[/red] \u8bf7\u63d0\u4f9b\u6587\u4ef6\u8def\u5f84\u6216 --text \u53c2\u6570")
        return
    export_formats = [f.strip() for f in export_fmt.split(",") if f.strip()] if export_fmt else None
    with console.status("[bold green]AI \u589e\u5f3a\u6d41\u6c34\u7ebf\u8fd0\u884c\u4e2d...[/bold green]"):
        result = run_pipeline(
            file_path=file, text=text or None, project_name=project_name,
            top_n=top, db_name=db_name, report_output=output,
            export_formats=export_formats, use_ai=True, ai_engine=engine,
            translate_mode=translate_mode,
        )
    console.print(format_pipeline_result(result))
    if result.glossary_json:
        console.print(f"\n[green]\u2713[/green] Glossary JSON \u5df2\u5bfc\u51fa\u81f3 {project_name}-glossary.json")
